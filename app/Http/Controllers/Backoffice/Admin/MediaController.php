...
class MediaController extends Controller
{
    public function index(Request $request): View
    {
        ...
        return view('backoffice.admin.media.index', [
            ...
            'breadcrumb' => [['text' => 'Media', 'url' => null]],
            'media' => $this->mediaQuery($request)->paginate(24)->withQueryString(),
            'stats' => $this->stats(),
            'filters' => $request->only(['search', 'type', 'collection', 'disk']),
            'isTrash' => false,
        ]);
    }
    ...
    public function trash(Request $request): View
    {
        ...
        return view('backoffice.admin.media.trash', [
            ...
            'media' => $this->mediaQuery($request, true)->paginate(24)->withQueryString(),
            'filters' => $request->only(['search', 'type', 'collection', 'disk']),
            'isTrash' => true,
        ]);
    }
    ...
    public function picker(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data' => $items->getCollection()->map(fn (Media $media) => $this->pickerPayload($media))->values(),
            'pagination' => [
    ...
    public function forceDelete(Request $request, int $media): RedirectResponse
    {
    ...
    public function bulkAction(Request $request): RedirectResponse
    {
        ...
        $this->authorizeAction(match ($validated['action']) {
            'delete' => 'media.delete',
        ...
        $processed = 0;
        foreach (array_unique(array_map('intval', $validated['media_ids'])) as $id) {
            $media = $validated['action'] === 'delete'
                ? Media::query()->find($id)
                : Media::onlyTrashed()->find($id);
        ...
        return back()->with('success', sprintf('%d media file(s) processed.', $processed));
    }
        ...
        return ($trash ? Media::onlyTrashed() : Media::query())
            ...
            ->when($request->filled('type'), function ($query) use ($request): void {
                ...
                $query->when($type === 'image', fn ($q) => $q->where('mime_type', 'like', 'image/%'))
                    ...
                    ->when($type === 'other', fn ($q) => $q->whereNotIn('id', function ($sub) {
                        $sub->select('id')->from('media')->where('mime_type', 'like', 'image/%')->orWhere('mime_type', 'like', 'video/%');
                    }));
            })
