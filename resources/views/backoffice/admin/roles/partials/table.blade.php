                ...
                <tr>
                    ...
                    <td class="text-right text-nowrap">
                        @if($isTrash)
                            ...
                            @endcan
                        @else
                            @can('roles.view')<button type="button" class="btn btn-outline-info btn-sm" data-toggle="modal" data-target="#showRoleModal{{ $role->id }}"><i class="fas fa-eye"></i></button>@endcan
                            @can('roles.manage')<a href="{{ route('admin.roles.edit', $role) }}" class="btn btn-outline-primary btn-sm"><i class="fas fa-edit"></i></a>@endcan
                            @can('roles.manage') @if(! $role->isProtected())<form action="{{ route('admin.roles.destroy', $role) }}" method="POST" class="d-inline" data-confirm="Move this role to trash?">@csrf @method('DELETE')<button class="btn btn-outline-danger btn-sm"><i class="fas fa-trash"></i></button></form>@endif @endcan
                        @endif
                    </td>
                </tr>
