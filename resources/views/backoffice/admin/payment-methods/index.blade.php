...
@section('meta_title', 'Payment Methods')

@section('page_content')
    <div class="row align-items-center justify-content-between mb-3">
        <div class="col-12 col-md-auto mb-2 mb-md-0 d-flex flex-wrap gap-2">
            @can('payment-methods.manage')
                <button type="button" class="btn btn-primary shadow-sm" id="btnCreateMethod">
                    <i class="fas fa-plus-circle mr-1"></i> Add New Method
                </button>
            @endcan
            <a href="{{ route('admin.payment-methods.trash') }}" class="btn btn-outline-danger shadow-sm ml-2">
                <i class="fas fa-trash-alt mr-1"></i> Trash Bin
            </a>
        </div>

        <div class="col-12 col-md-auto">
            <form id="bulkActionForm" action="{{ route('admin.payment-methods.bulk-action') }}" method="POST" class="form-inline">
                @csrf
                <div class="input-group">
                    <select name="action" class="custom-select custom-select-sm" required>
                        <option value="">Bulk Actions</option>
                        @can('payment-methods.delete')<option value="delete">Move to Trash</option>@endcan
    ...
    <div class="card card-outline card-primary shadow-sm">
        <div class="card-header bg-white py-3">
            <form id="filterForm" method="GET" action="{{ route('admin.payment-methods.index') }}" class="row g-2 align-items-center">
                ...
                <div class="col-12 col-md-6 mb-2 mb-md-0">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="search" name="search" id="search" class="form-control border-left-0" placeholder="Search by name, code or number...">
                    </div>
                </div>
        ...
        <div class="card-body p-0 position-relative">
            ...
            <div id="tableContainer">
                @include('backoffice.admin.payment-methods.partials.table', ['isTrash' => false])
            </div>
        </div>
    ...
    @include('backoffice.admin.payment-methods.partials.form')
    @include('backoffice.admin.payment-methods.partials.show')
...
@push('js')
    @include('backoffice.admin.payment-methods.partials.script', ['fetchUrl' => route('admin.payment-methods.index')])
@endpush